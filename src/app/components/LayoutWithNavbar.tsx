...
const LayoutWithNavbar = ({ children }: { children: React.ReactNode }) => {
  ...
  const fetchUserChats = async () => {
    if (session?.user.id) {
      fetch(`/api/userchats?userId=${session?.user.id}`, {
        ...
        .then((data: { data: any }) => {
          console.log(
            "groupByTime(data?.data?.chats)",
            groupByTime(data?.data[0]?.chats)
          );
  ...
  return (
    <div className="flex flex-row h-full relative">
      {/* The <aside> is always present in the DOM, but we control its visibility and position */}
      <aside
      ...
      >
        ...
        <div className="m-[22px] text-base">
          {recent &&
            Object?.keys(recent).map((ele) => (
              <div className="group my-4" key={ele}>
                <div className="text-xs text-primary my-2 p-2"> {ele} </div>
                {recent[ele].map((e: any) => (
                  <div
                    className="text-base text-primary my-2 p-2 hover:bg-slate-200 rounded-md"
                    key={e.title}
                  >
                    {e.title}
                  </div>
